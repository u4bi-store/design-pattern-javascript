/* 커맨드 패턴은 인자에 객체의 형태로 캡슐화하여 나중에 이용할 수 있도록
   펀션 이름 매개변수등 필요한 정보를 저장하거나 로그를 남기거나 취소할 수 있게하는 패턴임

   즉 요구하는 사항을 객체로 캡슐화 하여서 다른 요구사향을 집어넣거나 할 수 있음
   또한 요청 사항을 큐에 저장하거나 기록도하고 작업을 취소할 수 도 있음
   
   그러니깐 요청을 하는 객체와 그 요청을 수행하는 객체를 분리하고 싶다면
   커맨드 패턴이 어울린다고 함
*/
...
Calculator.prototype = { /* 프로토타입 지정 */
    execute : function(command){ /* 익스큐트 인자로 객체 받음 */
        ...
        /*  익스큐트를 호출할 때
            인자로 들어간 객체의 인자값과
            this._currentValue(현재값)을
            인자로 들어간 객체로 반환을 하고 로직이 구성되는데
            구성된 후의 반환이 된 값은 this._currentValue(현재값)으로 정의됨
        */
    ...
    undo : function(){ /* 언도 펀션을 둠 */
        ...
        /* https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Global_Objects/Array/pop
           ...
           그 값을 호출자에게 반홤함
        */
        this._currentValue = cmd.undo(this._currentValue); /* 반환하여 담음 */
        /* 반환한 해당 객체의
           this시점내 _currentValue를
...
/* calc 익스튜트는 위 객체를 인자로 받음
   ...
   this 시점의 즉 calc의 _currentValue에
   직접적으로 정의가 된다.
*/
console.log(calc.getCurrentValue());
/* 그 후 clac의 getCurrentValue() 펀션을 호출할 때
   변경되어져 있는 _currentValue를
   확인할 수 있다.
...
calc.undo(); /* 언도를 하게될 시
  ...
  이 (@)언도 펀션은
  그 반환된 객체내의
  두번째 담겨진 인자(언도에 담길값이라고 주석단 부분의)펀션으로
  그에 해당되는 로직이 구성되어 값을 반환하는데
  
  이 값이
  현재 calc객체의
  _currentValue로 지정이 된다.
*/
